...
local genBlocks = function()
    ...
    for i = 1, #data.folders.images do
        M.folder(data.folders.images[i][1], #M.group.blocks + 1, i, data.folders.images[i][3])

        if not data.folders.images[i][3] then
            for j = 1, #data.folders.images[i][2] do
                M.new(data.folders.images[i][2][j][1], #M.group.blocks + 1, data.folders.images[i][2][j][2], data.folders.images[i][2][j][3])
            end
        end

        index = index + #data.folders.images[i][2]
    end
end

M.folder = function(title, index, indexFolder, commentFolder)
    BLOCK.new(title, M.scroll, M.group, 'images', index, nil, nil, false, indexFolder, commentFolder)
end
