...
local genBlocks = function()
    ...
    for i = 1, #data.folders.sounds do
        M.folder(data.folders.sounds[i][1], #M.group.blocks + 1, i, data.folders.sounds[i][3])

        if not data.folders.sounds[i][3] then
            for j = 1, #data.folders.sounds[i][2] do
                M.new(data.folders.sounds[i][2][j][1], #M.group.blocks + 1, data.folders.sounds[i][2][j][2])
            end
        end

        index = index + #data.folders.sounds[i][2]
    end
end

M.folder = function(title, index, indexFolder, commentFolder)
    BLOCK.new(title, M.scroll, M.group, 'sounds', index, nil, nil, false, indexFolder, commentFolder)
end
...
M.create = function()
    ...
    local title = display.newText(STR['program.sounds'], ZERO_X + 40, ZERO_Y + 30, 'ubuntu', 50)
        ...
        title:addEventListener('touch', require 'Core.Interfaces.sounds')
