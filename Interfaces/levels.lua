...
local genBlocks = function()
    ...
    for i = 1, #data.folders.levels do
        M.folder(data.folders.levels[i][1], #M.group.blocks + 1, i, data.folders.levels[i][3])

        if not data.folders.levels[i][3] then
            for j = 1, #data.folders.levels[i][2] do
                M.new(data.folders.levels[i][2][j][1], #M.group.blocks + 1, data.folders.levels[i][2][j][2])
            end
        end

        index = index + #data.folders.levels[i][2]
    end
end

M.folder = function(title, index, indexFolder, commentFolder)
    BLOCK.new(title, M.scroll, M.group, 'levels', index, nil, nil, false, indexFolder, commentFolder)
end

M.new = function(title, index, link)
    BLOCK.new(title, M.scroll, M.group, 'levels', index, nil, link)
end
...
M.create = function()
    ...
    local title = display.newText(STR['program.levels'], ZERO_X + 40, ZERO_Y + 30, 'ubuntu', 50)
        title:setFillColor(unpack(LOCAL.themes.text))
        title.anchorX = 0
        title.anchorY = 0
        title.button = 'but_title'
        title:addEventListener('touch', require 'Core.Interfaces.levels')
    ...
    local but_add = display.newImage(THEMES.add(), ZERO_X + 190, MAX_Y - 95)
        ...
        but_add:addEventListener('touch', require 'Core.Interfaces.levels')
    M.group:insert(but_add)

    local but_play = display.newImage(THEMES.play(), MAX_X - 190, MAX_Y - 95)
        but_play.alpha = 0.9
        but_play.button = 'but_play'
        but_play:addEventListener('touch', require 'Core.Interfaces.levels')
    ...
    local but_list = display.newImage(THEMES.listopenbut(), MAX_X - 80, ZERO_Y + 62)
        ...
        but_list:addEventListener('touch', require 'Core.Interfaces.levels')
    ...
    local but_okay = display.newImage(THEMES.okay(), MAX_X - 190, MAX_Y - 95)
        ...
        but_okay:addEventListener('touch', require 'Core.Interfaces.levels')
